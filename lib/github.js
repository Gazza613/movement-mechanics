...
/**
 * Find the pull request Claude opened for a given issue.
 *
 * Matching on "#12" in the PR body assumes Claude always references the issue,
 * which it usually does but is not guaranteed - and when it doesn't, the
 * request sticks on "Claude is working on it" forever even though the work is
 * done. The branch name is the dependable signal: claude-code-action names its
 * branch after the issue it is answering (claude/issue-12-...).
 */
/**
 * Find the branch Claude pushed for an issue (claude/issue-5-20260713-0637).
 */
export function findBranch(branches, issueNumber) {
  const re = new RegExp(`^claude/issue[-_]${Number(issueNumber)}\\b`, "i");
  const hit = (branches || []).find((b) => re.test(b.name || ""));
  return hit ? hit.name : null;
}

/**
 * Open the pull request for a finished branch.
 *
 * claude-code-action does NOT open a PR when it answers an issue - it pushes
 * the branch and hands back a "Create PR" link for a human to click. That is
 * fine on GitHub and useless in a dashboard, where the client never sees the
 * link and the request appears stuck forever. So we open it ourselves, which
 * also means we don't depend on the agent choosing to do it.
 */
export async function openPr(branch, issue) {
  return gh(`/repos/${repo()}/pulls`, {
    ...
    body: JSON.stringify({
      ...
      head: branch,
      base: "main",
      body: `Requested from the Movement Mechanics editor panel.\n\nCloses #${issue.number}`,
    }),
  });
}

export function findPr(pulls, issueNumber) {
  const n = Number(issueNumber);
  const byBranch = (pulls || []).find((p) =>
    new RegExp(`issue[-_]${n}\\b`, "i").test((p.head && p.head.ref) || ""),
  );
  if (byBranch) return byBranch;

  return (pulls || []).find((p) =>
    new RegExp(`#${n}\\b`).test(`${p.body || ""} ${p.title || ""}`),
  );
}
