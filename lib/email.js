...
/**
 * Build a safe Reply-To header.
 *
 * The naive `${name} <${email}>` breaks whenever the name contains an angle
 * bracket: the header becomes malformed, Resend rejects the send outright,
 * and the visitor sees a generic error while their enquiry is silently lost.
 * Strip the characters RFC 5322 forbids in a display name, then quote what's
 * left so commas and full stops are safe too. Falls back to the bare address
 * if nothing usable survives.
 */
export function replyTo(name, email) {
 ...
 * @param {string} [o.badge]      Optional status label (used for the CV attachment note)
 * @param {string} [o.badgeState] "attached" | "warning" | "none" - drives the label's colour
...
export function renderEmail(o) {
  ...
  // Attachment status. Deliberately NOT styled as a button: an email body
  // cannot open an attachment (there is no URL to point at - it lives in the
  // client's attachment tray), so anything button-shaped here is a lie. This
  // is a status label, and it looks like one.
  const badgeStyles = {
    attached: { bg: "rgba(135,255,0,0.06)", border: "rgba(135,255,0,0.25)", fg: LIME, icon: "&#128206;" },
    warning: { bg: "rgba(255,176,0,0.06)", border: "rgba(255,176,0,0.3)", fg: "#ffb000", icon: "&#9888;" },
    none: { bg: PANEL, border: BORDER, fg: GREY_DIM, icon: "&#8212;" },
  };

  let badge = "";
  if (o.badge) {
    const s = badgeStyles[o.badgeState] || badgeStyles.none;
    const hint =
      o.badgeState === "attached"
        ? `<span style="color:${GREY_DIM};font-weight:400;"> &nbsp;&middot;&nbsp; see the attachment on this email</span>`
        : "";

    badge = `
      <tr>
        <td style="padding:0 0 24px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:${s.bg};border:1px solid ${s.border};border-radius:8px;">
            <tr>
              <td style="padding:13px 16px;font-family:${FONT_BODY};font-size:13px;line-height:20px;font-weight:600;color:${s.fg};">
                ${s.icon}&nbsp; ${esc(o.badge)}${hint}
              </td>
            </tr>
          </table>
        </td>
